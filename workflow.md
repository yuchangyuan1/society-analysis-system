...
### 1.1 社区数据臂 — `python main.py --jsonl ... | --subreddit ... | --channel ...`
...
| `tools/topic_resolver.py::TopicResolver` | 用户话题短语 → topic_id |
...
| `services/reddit_service.py / telegram_service.py` | 社区数据源 |
...
| 页面 | 用途 |
|---|---|
| `ui/pages/0_Chat.py` | 三栏 Chat 页（侧栏 + 对话 + 5 Tab 分析面板） |
| `ui/pages/1_Run_List.py` | 列出后台 run |
| `ui/pages/2_Run_Detail.py` | 单 run 详情 |
| `ui/pages/3_Reflection.py` | Reflection 看板（Chroma 2 / Chroma 3 / audit log） |
...
- **`ui/pages/2_Run_Detail.py`** 还在显示 v1 字段名，未做 v2 适配（不影响 Chat）。
